...
- **Kiro IDE** 已安装并正常运行
- **MCP 服务可用**：`loeyae-skills` MCP 服务（`https://mcp-skills.dev.loeyae.com/sse`）需要网络可达；`awesome-design` MCP 服务（`https://mcp-design.dev.loeyae.com/sse`）可选（UI Mock 设计风格选择时使用）
