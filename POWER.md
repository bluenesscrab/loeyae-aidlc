...
keywords: ["aidlc", "AI-DLC", "继续上次的工作", "认领单元", "团队协作模式", "loeyae", "功能设计", "用户故事", "架构设计", "单元生成", "代码审查", "逆向工程", "根因分析"]
author: "Loeyae Team"
---
...
### Operations 阶段
- `operations-operations.md` — CI/CD 配置生成与部署文档（Dockerfile + Jenkinsfile + K8s 部署清单）
...
**症状**：调用 `get_skill_content` 或 `search_skill` 时报错 "Connection refused" 或超时

**解决方案**：
1. 确认网络可达：检查是否能访问 `https://mcp-skills.dev.loeyae.com/sse`
