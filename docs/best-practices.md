...
| 2 | 架构师 | 技术可行性、系统边界、集成点、性能约束 | 标准+全面深度 |
| 3 | 开发者 | 数据模型、接口定义、边界条件、错误处理 | 仅全面深度 |
| 4 | 测试人员 | 测试场景、边界用例、异常流程、验收标准可测试性 | 仅全面深度 |
| 5 | 全局一致性 | 跨角色矛盾、前后不一致、遗漏补充 | 仅在发现矛盾时触发 |
...
当 AI 在第5轮提出跨角色矛盾时，需要你做出明确的取舍决策：

```
AI: 产品要求"实时推送订单状态变更"，但架构视角确认当前基础设施不支持 WebSocket。
    A) 降级为轮询方案（每 5 秒查询一次）
    B) 增加 WebSocket 基础设施投入
    C) MVP 阶段用轮询，后续迭代升级为 WebSocket

[回答]: C
原因：MVP 时间紧张，轮询方案可以快速实现。在 state.md 中记录为技术债务，
第二期迭代时升级。
```
...
每个阶段/步骤完成时，AI 会生成一个可复制的"新 Session 提示词"，方便你在新会话中继续工作。
...
AI 完成一个阶段后会展示类似这样的提示词：

```
📋 新 Session 提示词（复制到新对话即可继续）：

使用 AI-DLC，继续 order-management 模块的开发。

当前状态：
- 阶段：CONSTRUCTION
- 已完成：单元 order-service 代码生成+审查通过
- 下一步：单元 order-query 开发
- 架构模式：多模块
- 活跃模块：order-management

请读取 state.md 恢复上下文，从「order-query」单元开始。
```
...
```
┌─────────────────────────────────────────────────────────────┐
│              产品级 Inception（团队一起，轻量）                 │
│                                                             │
│  PM + Architect 协作（1-2 次会话）                            │
│  ──────────────────────────────                             │
│  工作区检测 → 产品需求概览 → 模块划分 → 模块间接口契约        │
│                                                             │
│  产出物：product-overview.md + modules.md + contracts.md     │
│  （总计 ~5-10KB，极轻量）                                    │
└─────────────────────────────────────────────────────────────┘
                          │
           ┌──────────────┼──────────────┐
...
│  Developer A   │ │  独立会话      │ │  独立会话      │
│  独立会话      │ │                │ │                │
│                │ │  模块级        │ │  模块级        │
│  模块级        │ │  Inception     │ │  Inception     │
│  Inception     │ │      ↓         │ │      ↓         │
│      ↓         │ │  Construction  │ │  Construction  │
│  Construction  │ │  (单元认领)    │ │  (单元认领)    │
│                │ │                │ │                │
└────────────────┘ └────────────────┘ └────────────────┘
        │                  │                  │
        └──────── PR 合并到主分支 ─────────────┘
