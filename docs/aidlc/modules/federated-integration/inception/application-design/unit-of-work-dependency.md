...
- **版本边界**：Provider v3 OpenAPI `3.0.0-candidate.1`，路由前缀 `/v3`
...
| `U-C02-CORE-CONTEXT-DOCUMENT` | `U-C01-CORE-PROVIDER-CLIENT` + Provider 检索/上下文/血缘/逆向接口 | 平台无关 Gateway、固定修订/片段、失败决策 | U-C01-CORE-PROVIDER-CLIENT 端口和 Provider v3 候选契约稳定后可与 Provider Portal 并行 | Gateway 语义未冻结、固定引用不可解析、事实分层规则冲突 | Core 文档、引用、血缘、逆向说明与 state v2 恢复；不消费讨论摘要 |
| `U-C03-KIRO-ADAPTER` | `U-C02-CORE-CONTEXT-DOCUMENT` | 共享 Core 命令/结果、凭据注入端口、状态呈现模型 | Core API 冻结后与 U-C04-CLAUDE-ADAPTER/U-C05-OPENCODE-ADAPTER 并行 | 需要平台专属业务分支、Provider 直连、讨论摘要或第二份 state | Kiro 适配与跨平台一致性 |
...
| `U-C06-CROSS-SERVICE-TESTS` | Provider/Consumer 全部生产单元 | 固定版本、真实环境、Core/平台端口、报告位置 | 可先准备追踪设计；不得提前形成运行通过结论 | 12 个运行锚点未闭合、接口/版本未冻结、无执行授权 | 契约、Legacy、双项目、滥用治理、状态/内容回滚、版本组合与三平台一致性稳定证据 |
...
|------|-------|-------------------|----------|
| Provider 字段与错误 Schema | SSOT Provider | U-C01-CORE-PROVIDER-CLIENT 从 OpenAPI 生成或自动校验 v3 八项读取/Context/血缘/逆向写 DTO；不向 Core 或平台暴露 `createMaterial`、`archiveMaterial`、`restoreMaterialStatus`、`restoreMaterialRevision` | Consumer 手写、复制或扩展 Provider Schema |
...
3. 所有在线调用固定 `X-SSOT-Contract-Version: 3.0.0-candidate.1`、使用 `/v3` 并传播 correlationId；其他版本以 `CONTRACT_VERSION_UNSUPPORTED` 阻断，未验证组合不得宣称兼容。
...
- 当前静态设计未发现跨 `service_id` 的生产修改范围重叠；三平台仍只经共享 Core、Provider 零直连且无讨论摘要；运行关系仍未验证。
- CR4 B4 复验确认 6 个 Consumer 单元的 `unit_id`、`service_id`、主归属、依赖方向和数量均不变，无需重生成工作单元。
