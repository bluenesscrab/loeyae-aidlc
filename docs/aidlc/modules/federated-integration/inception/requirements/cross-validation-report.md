...
## 11. CR-U-P01-DISCUSSION-SCOPE-001 CR4 B1 I5/I6/I7/I8 Consumer 复审

- **复审时间**：2026-07-22T08:42:10Z
- **批准前提**：Provider 权威 073=A、074=A
- **复审对象**：Consumer 需求、共享需求、决策摘要、故事主文件、U-C01/U-C02 需求与故事切片
- **复审状态**：Provider-first B1 文档候选静态复审通过；等待 Provider 权威 `CR4-U-P01-DISCUSSION-SCOPE-I5-I8-APPROVAL-075`
- **执行边界**：未修改 Consumer 应用设计、DTO、I13/I14、平台适配、实现或配置；未构建、测试、执行 C8、提交或推送

### 11.1 历史与 Provider-first 边界

本文件第 1—10 节原样保留。Consumer 只在 Provider B1 范围确定后同步资料消费语义，不超前修改机器契约。Consumer 既有两份 I13 差异 `_index.md` 与 `consumer-aggregated-test-cases.md` 保持前序状态，未被本轮覆盖。

### 11.2 审查项 b：Consumer 需求 ↔ Provider B1

| # | 检查点 | 结果 | 结论 |
|---|--------|------|------|
| 1 | Provider-first | ✅ | Provider 先将讨论域退役为候选，Consumer 随后移除 Context/正式文档对讨论摘要和 Conclusion 的依赖 |
| 2 | ADLC-FR-005 | ✅ | ContextBundle 只固定资料、修订、片段、来源、索引/降级与预算，不依赖讨论对象或状态 |
| 3 | ADLC-FR-006 | ✅ | 正式文档区分有固定资料引用支撑的内容和无来源模型推断，不再区分未确认评论/显式结论 |
| 4 | 共享 NFR/约束 | ✅ | 引用完整性、数据最小化、state v2、Legacy、平台中立和外部事实边界保持；新增零讨论对象/摘要负向范围 |
| 5 | 单元边界 | ✅ | U-C02 同步需求语义；U-C01 仅记录边界，不修改字段、DTO、版本协商或错误映射 |

**审查项 b 结果**：5/5 通过。

### 11.3 审查项 c：Consumer 故事 ↔ 需求

| # | 检查点 | 结果 | 结论 |
|---|--------|------|------|
| 1 | FR 覆盖 | ✅ | `ADLC-FR-001—013` 13/13 继续有唯一主故事和 Gherkin |
| 2 | 005 对齐 | ✅ | ADLC-US-005 保留固定来源、索引/降级、预算及异常阻断，不再承载讨论状态 |
| 3 | 006 对齐 | ✅ | ADLC-US-006 以固定资料引用与模型推断二层语义替代四类讨论事实状态 |
| 4 | 角色/优先级 | ✅ | 五类画像与 Must 优先级不变，未新增角色、人员 Owner 或工作单元 |
| 5 | 负向范围 | ✅ | Consumer 不创建、不读取、不缓存、不推断讨论对象或摘要；未引入 Provider 内部实现或字段 Schema |
...
| # | 检查点 | 结果 | 结论 |
|---|--------|------|------|
| 1 | 核心旅程 | ✅ | state v2 → 角色/目标 → Provider 项目解析 → 资料选择 → Context → 正式文档 → 引用/血缘闭合 |
| 2 | 资料能力保护 | ✅ | 默认最新、显式包含/排除/旧版、索引降级、固定引用、血缘和逆向说明能力均保留 |
| 3 | 异常路径 | ✅ | 项目冲突/越权、低置信/来源冲突、固定修订/索引/Provider/血缘失败和 Legacy 零调用边界均保留 |
| 4 | U-C01/U-C02 | ✅ | U-C02 承接语义变化；U-C01 后续字段/契约适配明确留待 B2，不新增故事 |
| 5 | 新发现需求 | ✅ | 未发现 B1 范围外新需求；`service_id`、主归属和单元数量不变，停止条件未触发 |

**审查项 d 结果**：5/5 通过；新发现需求 0。

### 11.5 运行与门禁结论

- Consumer 文档对齐：静态候选 PASS；运行未验证。
- 057 保持 `0/12`，全部 Consumer UC-D 继续 blocked。
- **整体结论**：Consumer B1 I5/I6/I7/I8 候选与 Provider 文档语义闭合；等待 075。075 批准前不得进入 B2、修改 DTO/应用设计/I13/I14、实施代码或声明运行通过。

## 12. CR-U-P01-CROSS-PROJECT-IMPORT-001 CR4 B1 Consumer I5/I6/I7/I8 复审轮次

- **复审时间**：2026-07-24T02:26:12Z
- **批准前提**：`CR2-U-P01-CROSS-PROJECT-IMPORT-IMPACT-APPROVAL-080=A`、`CR3-U-P01-CROSS-PROJECT-IMPORT-PLAN-APPROVAL-081=A`、`CR4-U-P01-CROSS-PROJECT-IMPORT-B0-APPROVAL-082=A`
- **复审对象**：Consumer 需求主文件、数据模型、决策摘要、共享需求、U-C01/U-C02/U-C06 需求切片、故事主文件、共享故事及对应故事切片
- **复审状态**：B1 Consumer I5/I6/I7/I8 文档候选静态复审通过；Provider-first 已稳定，等待后续独立严格门禁
- **执行边界**：仅更新批准范围内的 Consumer 需求、故事与本报告；未修改 product contracts、I12/I13/I14、配置或实现，未构建、测试、执行 C8、提交或推送

### 12.1 历史保护

本文件第 1—11 节原样保留。本节仅追加本 CR 的 B1 Consumer 静态复审证据；既有审批、契约、设计、I13/I14 和运行状态不被覆盖或改写。

### 12.2 Provider-first、对象、状态与执行边界

| 边界 | 结果 | 复审结论 |
|------|------|----------|
| Provider-first | ✅ | Provider 的三类写入例外、状态与恢复语义已稳定形成 B1 候选，Consumer 未超前定义机器契约 |
| 对象边界 | ✅ | 跨项目写入后的资料仍唯一归属有效目标项目；Consumer 不复制 Provider 权威资料或新增独立业务项目写入逻辑 |
| 状态边界 | ✅ | 导入可选 `active`/`archived`，支持双向状态转换；历史内容恢复创建新不可变修订并条件推进唯一当前指针，旧引用与血缘不漂移 |
| 执行边界 | ✅ | 仅导入、归档、恢复允许成功 Bearer 主体免目标项目成员与项目级授权；匿名写入禁止；Consumer 所有读取及派生能力继续目标项目隔离 |
| 零泄露与历史保护 | ✅ | 成功、失败、冲突、重复和状态转换均不得泄露目标项目既有事实；第 1—11 节及受保护产物保持不变 |

### 12.3 I6 审查项 b：Consumer 需求 ↔ 已批准范围

| # | 检查点 | 结果 | 结论 |
|---|--------|------|------|
| 1 | 080/081/082 与 Provider-first | ✅ | 三类写入例外、读取隔离与 Provider-first 顺序完整映射，无扩大解释 |
| 2 | U-C01 边界 | ✅ | 只记录未来 Provider 新主版本适配意图和读取隔离不退化；机器契约及兼容治理细节留待 B2 |
| 3 | U-C02 边界 | ✅ | 目标项目读取授权、固定修订/片段、Context 与血缘隔离保持，不因写入成功产生读取权 |
| 4 | U-C06 治理覆盖 | ✅ | 双项目、滥用、配额/成本/风险/通知/申诉/隔离处置/审计及 Provider/Consumer 组合均进入候选验证范围 |
| 5 | 负向范围 | ✅ | 未新增 Consumer 独立写入逻辑，未修改 product contracts、I12/I13/I14、配置或实现，未提前定义 B2 机器细节 |

**I6 审查项 b 结果**：5/5 通过。

### 12.4 I8 审查项 c：Consumer 故事 ↔ 需求

| # | 检查点 | 结果 | 结论 |
|---|--------|------|------|
| 1 | FR 与故事覆盖 | ✅ | 现行 FR/故事编号和主归属保持；U-C01/U-C02/U-C06 候选语义均有 Gherkin 支撑 |
| 2 | 读取与固定引用 | ✅ | 故事保持目标项目读取授权，固定具体修订与片段，历史恢复后的既有引用不漂移 |
| 3 | 零泄露与治理 | ✅ | 双项目、滥用治理及成功/失败/冲突/重复/状态转换零泄露均有故事验收路径 |
| 4 | 三平台边界 | ✅ | Kiro、Claude Code、OpenCode 只经 Core 使用 Provider，平台直连或复制业务规则均判定失败 |
| 5 | 范围边界 | ✅ | 故事未引入 Consumer 独立写入、跨项目读取或 B2/I12/I13/I14/实现细节 |

**I8 审查项 c 结果**：5/5 通过。

### 12.5 I8 审查项 d：基于 Consumer 故事评估需求完整性

| # | 检查点 | 结果 | 结论 |
|---|--------|------|------|
| 1 | 双项目核心旅程 | ✅ | Provider 写入例外 → Consumer 目标项目授权读取 → 固定 Context/引用/血缘 → 三平台 Core 闭环完整 |
| 2 | 隐含前置条件 | ✅ | Bearer、有效目标项目、读取授权、固定修订/片段、Provider/Consumer 组合和 Core 调用边界均有需求支撑 |
| 3 | 异常与滥用路径 | ✅ | 匿名、无读取权、零泄露、配额/成本、风险/通知/申诉、隔离处置和审计均被覆盖 |
| 4 | 历史保护与平台拓扑 | ✅ | 历史内容恢复不改旧引用；三平台无直连，不产生平台独立业务状态或写入逻辑 |
| 5 | 新发现需求 | ✅ | 新发现需求=0；机器契约、设计、I13/I14、配置和实现继续留待后续门禁 |

**I8 审查项 d 结果**：5/5 通过；新发现需求=0。

### 12.6 运行与门禁结论

- V-XPIMPORT-01、03—06、08—09 的 B1 Consumer 文档可执行部分：静态候选 PASS；机器契约、设计、实现和运行部分未验证。
- V-XPIMPORT-02、07、10—12 尚未执行，不得提前标记；057 保持 `0/12`，全部 Consumer UC-D 继续 blocked。
- product contracts 与 I13/I14 只进行哈希复核，不改写内容；Consumer 不进入 B2，不新增独立业务项目写入逻辑。
- **整体结论**：B1 Consumer I5/I6/I7/I8 文档候选静态复审通过；Provider-first 已稳定。运行能力、构建、测试与 C8 均未执行、未验证；后续严格门禁批准前不得修改机器契约、product contracts、I12/I13/I14、配置或实现。
