...
- **状态**：`CR-U-P01-CROSS-PROJECT-IMPORT-001 / CR4 B1 Consumer` 候选已回写；Provider-first 已稳定，等待后续独立严格门禁
- **风险**：L4
...
| 讨论使用 | Consumer 首期不创建、不读取、不缓存、不推断讨论对象或摘要；有来源内容依赖固定资料修订/片段引用，无来源内容标识为模型推断 | `C1-U-P01-Q012=X`、073=A、074=A | B1 候选，等待 075 |
...
5. 固定 ContextBundle 的资料修订、片段、来源、索引和降级状态。
6. 在项目工作区生成正式文档并区分有固定资料引用支撑的内容与无来源模型推断。
...
| ContextBundle | SSOT 构建 | 消费固定 bundle，保留来源、索引和降级状态；不依赖讨论对象或摘要 | 未验证 |
...
- **契约缺口**：Provider 新主版本适配、机器契约内容和兼容治理留待 B2；本批只冻结 Consumer 意图与读取隔离不退化。
- **B1 边界**：U-C01 只记录未来 Provider 新主版本适配意图；U-C02 保持目标项目读取授权和固定修订/片段；U-C06 扩展双项目、滥用治理、版本组合及三平台经 Core 验证。
...
## 8. CR-U-P01-CROSS-PROJECT-IMPORT-001 B1 Consumer 决策

1. 接受已稳定 Provider-first 语义：仅导入、归档与恢复允许成功 Bearer 主体免目标项目成员资格和项目级授权写入任意有效目标项目；写入禁止匿名并保持结果与失败零泄露。
2. Consumer 不新增独立业务项目写入逻辑，也不复制导入状态、双向状态转换或历史内容恢复规则；这些规则仍由 Provider 权威管理。
3. U-C01 只声明未来适配 Provider 新主版本，并以读取项目隔离不退化为硬约束；机器契约和兼容治理细节留待 B2。
4. U-C02 继续要求目标项目读取授权，固定资料修订与片段；Provider 恢复历史内容形成新不可变修订，不得改变既有引用和血缘。
5. U-C06 覆盖双项目、滥用与配额/成本/风险/通知/申诉/隔离处置/审计、Provider/Consumer 版本组合，并验证三平台只经 Core、不得直连 Provider。
6. 本批不修改 product contracts、I12/I13/I14、配置或实现，不形成构建、测试、C8 或运行通过结论。
