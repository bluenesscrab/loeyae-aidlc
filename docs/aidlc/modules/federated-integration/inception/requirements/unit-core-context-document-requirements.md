...
`MaterialSelection` 必须按角色、目标文档和提示词检索当前目标项目，且仅在当前主体具备该项目读取授权时继续；默认选择最新修订，用户可显式包含、排除或指定旧版。exclude 优先且排除项不得重新进入；范围过大、低置信、来源冲突或预算不足时必须展示原因并等待确认，未确认不得构建 ContextBundle。Provider 对导入、归档与恢复的写入例外不改变本读取前置条件。
...
Consumer 必须校验并消费 Provider 固定的 `ContextBundle`，保留项目、资料 ID、修订、片段、来源、索引状态、实际检索路径、降级和预算。跨项目、固定引用缺失、修订漂移或排除项命中必须阻断；GraphRAG 失败不能被解释为资料不存在。ContextBundle 不依赖或承载讨论对象、状态或摘要。
...
AI-DLC 必须在业务项目既有阶段目录生成正式研发文档，并明确区分有固定资料修订/片段引用支撑的内容与无来源模型推断。正文与批准版本归业务工作区/Git或既有文档库；无来源推断不得表述为资料事实，内容校验失败的草稿不能进入待审批基线。
...
每个由资料支撑的章节必须形成固定 `FragmentCitation`，并把 `DocumentSection` 与 `LineageRecord` 经 Provider 回写。引用必须包含实际使用的具体修订和片段，源资料更新或历史内容恢复创建新修订后，历史章节仍解析到原修订与片段；血缘 partial/失败时保留 Pending 和恢复动作，不标记 synced，也不删除本地正文。
...
U-C02-CORE-CONTEXT-DOCUMENT 直接承担 `ADLC-NFR-004`、`005`、`006`，并与 U-C01-CORE-PROVIDER-CLIENT/平台单元共同满足 `ADLC-NFR-001`、`003`、`007`；性能、预算和质量阈值属于 `ADLC-NFR-008`，当前无运行证据。所有 Provider 调用必须经 U-C01-CORE-PROVIDER-CLIENT，正式正文不转移到 Provider。Consumer 首期不创建、不读取、不缓存、不推断讨论对象或摘要；跨项目写入例外不改变目标项目读取授权、固定修订/片段和血缘隔离。U-C01 的机器契约适配留待 B2。
