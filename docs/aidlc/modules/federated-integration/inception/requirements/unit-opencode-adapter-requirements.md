# U-C05-OPENCODE-ADAPTER opencode-adapter 需求切片
...
OpenCode Plugin 必须把平台入口、短时凭据、调用结果、审批呈现和会话接力映射到共享 Core。其输入与 Kiro/Claude 相同时，RoleIntent、TargetDocument、MaterialSelection、ContextBundle、固定引用、血缘、错误决策和下一 state v2 位置必须一致。OpenCode 特有的插件入口与交互只能改变调用和呈现，不得复制或改写 Core 的项目解析、资料选择、降级、引用、血缘和恢复规则，也不得把平台名称写入业务事实。
