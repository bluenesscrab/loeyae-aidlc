...
- **作为**：需要生成目标文档的角色
- **我希望**：按角色和目标在具备读取授权的目标项目中自动检索，并能包含、排除或指定旧版
- **以便**：最终资料范围符合我的意图且每项选择理由可追踪，Provider 写入例外不改变可读范围
...
Scenario: 写入成功不产生目标项目读取权
  Given 已认证主体可以向目标项目写入但无该项目读取权限
  When AI-DLC 尝试为该主体检索资料或构建 ContextBundle
  Then U-C02 阻止读取和生成，不使用写入结果推断目标项目资料、成员、内容或派生事实
...
Scenario: 源资料更新或历史内容恢复后历史引用保持原修订
  Given 文档章节引用 R1/F1 且源资料随后形成 R2 或由历史内容恢复形成新修订
  When 用户检查原章节来源
  Then 章节引用仍解析到 R1/F1，只有新的生成任务在获得目标项目读取授权后默认使用新的当前修订
...
## CR-U-P01-CROSS-PROJECT-IMPORT-001 B1 边界

U-C02 不承接跨项目导入、归档、恢复或状态转换业务逻辑。所有资料读取、列表、检索、Context、血缘、逆向、解析与索引继续要求目标项目读取授权；选择、上下文、引用和血缘固定具体修订与片段，Provider 历史内容恢复形成新修订不得改变既有引用。U-C01 的机器契约适配留待 B2，本批不修改 product contracts、I12/I13/I14 或实现。
