...
- **目标用例目录**：`../application-design/test-cases/`；当前 Consumer 45 个 UC-D（35 个产品 + 10 个技术）已按本次 B4 静态重派生；权威 058=A 仅批准历史 44 个 Consumer UC-D，本次新增/重派生内容尚待 CR5 关闭审批

## Consumer 派生边界

1. 从本仓 13 个 Consumer 故事、35 个 Gherkin Scenario、四份已批准 Core Consumer 设计和 Provider OpenAPI 3.1 `3.0.0-candidate.1`（`/v3`） 派生用例。
...
|------|-----------|---------------|
| SSOT Provider 用户故事 | 11 个当前故事、37 个 Gherkin Scenario | 建立 Provider 前置条件、稳定错误与可观察状态 |
| AI-DLC Consumer 用户故事 | 13 个故事、35 个 Gherkin Scenario | 资料选择、ContextBundle、正式文档、引用/血缘、Legacy 与恢复 |
| 双仓合计 | 24 个当前故事、72 个 Gherkin Scenario | 建立需求—故事—设计—用例跨仓追踪 |
| Provider 机器契约 | OpenAPI 3.1，`3.0.0-candidate.1`，`/v3`，19 个 operationId | Consumer 类型、错误、重试与版本组合的唯一契约来源 |

采用 Provider 权威计划定义的 A—J 十个参数化用例簇，不执行 72 个当前 Scenario × 19 个 operations × 3 个平台的全笛卡尔展开。Consumer 重点承担以下切片：
...
- [x] 盘点双仓 24 个当前故事、72 个 Gherkin Scenario、Provider OpenAPI 候选和 19 个 operationId。
...
- [x] 生成并验证 Consumer/Core/三平台模块级测试用例正文及追踪矩阵：35 个产品 UC-D、10 个技术 UC-D，35/35 个源 Gherkin 场景逐字且唯一覆盖。
...
2026-07-20T03:25:22Z 已按 Boss 选择 A 修正阶段时序：057 仅控制运行解除，Consumer 45 个 UC-D 已按 `design_status=ready`、`execution_status=blocked`、`status=blocked` 生成并完成设计级验证。当前唯一审批入口是 Provider 权威 `CR4-B5-I13-TEST-CASE-APPROVAL-058`；批准前不关闭 I13、不启动 I14/Construction、不修改代码或平台适配，也不声明运行能力已验证。

## V-XPIMPORT-01—12 静态候选设计映射

V-XPIMPORT-01—12 已映射到 Consumer 35 个产品 UC-D、10 个技术 UC-D 与 6 个既有 I14 单元，覆盖权威意图、v3 机器契约、跨项目零泄露、状态/内容恢复、滥用治理、下游隔离、Portal 前置证据、Core 适配、三平台边界、I13/I14 追踪、未知事实盘点及非破坏性回滚。该映射仅是静态候选，全部 `execution_status=blocked`，不代表任一运行项通过；057 仍为 `0/12`。
...
2026-07-20T08:30:01Z 已回读 SSOT Provider 权威答案 058=A。Boss 批准双仓 I13 设计级测试用例与追踪结论；Consumer 45 个 UC-D 保持 `design_status=ready`、`execution_status=blocked`、`status=blocked`，057 继续以 0/12 阻断运行。双仓进入 I14 规划，但不授权生成 I14 产物、Construction、代码或平台适配。
