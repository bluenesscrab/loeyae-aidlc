...
| `CONS-ADLC-RECOVERY-001` | Provider 故障及 v3 状态/内容回滚后从同一 state v2 恢复；ADLC-FR-011/013 | 业务工作区 state v2 Owner 为 Core；Provider 管资料状态与不可变修订 | 失败不得推进依赖远端事实的步骤；状态恢复不改修订，历史内容恢复创建新修订且旧引用不变 | correlationId 与幂等键关联重试；不无限重试 | 当前步骤可见；用户修复配置/权限或稍后重试 | 审计失败、恢复动作、状态转换、新旧修订与最终远端状态 | 未验证 |
...
5. I13 必须派生超时但远端可能成功、重复提交、partial 血缘、重启恢复、Legacy 误调用、Provider/Consumer 版本组合、双项目隔离、滥用治理、状态/内容回滚和三平台一致性用例。
6. Legacy 对 Provider v3 八项读取契约及 `createMaterial`、`archiveMaterial`、`restoreMaterialStatus`、`restoreMaterialRevision` 四个生命周期写入口均须零调用；三平台只经 Core，且不暴露这四个写入口。
