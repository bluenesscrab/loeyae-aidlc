...
- **变更 ID**：CR-SCB-001
- **状态**：CR-SCB-001 规则已实施（`rules_implemented`）；端到端试运行验证待完成
...
- 枚举值声明；
- 由机器契约生成的类型；
- 必要的序列化或校验元数据，并执行对应结构验证。

声明型物化禁止：

- 空方法体；
- 返回 `null`、`false`、`0` 等伪默认值；
- 通过异常制造可编译的伪实现；
- 默认方法、构造器或静态方法中的业务逻辑；
- 数据访问、网络调用、状态转换；
- 为通过依赖注入而创建伪 Bean。

任何可执行行为仍执行正常 RED-GREEN-REFACTOR。
...
5. 执行分类型验证
   ...
   - 跨进程接口：机器契约检查 + 兼容性验证
   - 事件：Schema 校验
   失败：状态为 blocked，不解锁消费者

6. 双轴审查
   - Spec：签名、类型、路径与 I12/I14 一致
   - Standards：命名、包结构、无占位实现、无重复契约
   失败：修复并重审

7. 记录基线
   - state.md 写 verified
   - 记录实际验证命令和结果
   - 团队模式记录经用户批准产生的 commit/PR/SHA
   - 不自动创建 commit

8. 解锁
   - contract 类型依赖视为满足
   - implementation/runtime 类型继续等待
```
