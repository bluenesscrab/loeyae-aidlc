...
| 情况 | 表现 | 处理 |
|---|---|---|
| A. 实现 < 产品故事 | 已有代码基线的实现漏了或简化了产品故事的某个 Given/Then | 不是演化，是 bug。回 Construction 补实现，不走 CR |
| B. 实现 > 产品故事 | 已有代码基线包含产品故事未要求的功能 | ⚠️ 触发 CR。开发不得默默保留：产品确认要→CR4 就地回写产品故事；不要→删除多余实现 |
| C. 实现 ≠ 产品故事 | 已有代码基线改变了产品意图（如接口语义变了、Then 的业务结果变了） | 🔴 触发 CR（按高影响处理）。开发必须说明：原产品故事说 X，实现成了 Y，原因 Z。产品决策：接受 Y→回写；改回 X→返工 |

尚未取得 C8 通过证据且不属于存量已实现行为时，不按本表触发 CR；改为执行 `common-workflow-changes.md` 的在途产品协调。

### 实现期偏离的核心纪律

**禁止产出独立的"开发版用户故事/需求"**。已有代码基线与产品故事的偏离必须通过缺陷修复或 CR 回写产品版；尚无代码基线时通过在途协调同步产品版——产品版始终是唯一真相。
