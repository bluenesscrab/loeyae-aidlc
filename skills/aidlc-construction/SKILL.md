...
- 后端（Loeyae Boot）：MCP 渐进式披露加载（`get_skill_outline` → `get_skill_section` 优先，`get_skill_content` 兜底）+ `common-tech-security.md` + `common-database-design.md`
...
- 测试：`common-tech-testing.md`；Loeyae Boot 测试 → MCP `get_skill_section("loeyae-test", "测试基类")`
...
- **审查**：阶段 1 规格合规（完整/准确/无多余/接口契约）→ 阶段 2 代码质量（关键/重要/建议）+ 框架规范逐项对照（仅 Loeyae Boot）。顺序不可颠倒，发现问题必须修复并重新审查。（`construction-code-review.md` + `construction-code-generation.md`）
- **手术式变更**：审查发现的"建议改进"记录到 state.md 的"待优化项"，不在当前单元修复。（`core-workflow.md`）
- **单元级微型摘要**：每个单元审查通过后、进入下一单元前，必须输出实施摘要。（`construction-implementation-report.md`）
...
构建和测试通过后，必须生成阶段级完整报告（按 `construction-implementation-report.md`）。
