...
function main() {
  ...
  console.log("  loeyae-skills（渐进式披露 v2.0）:")
  console.log("  - get_skill_outline(name)      查看规范章节大纲")
  console.log("  - get_skill_section(name, section)  按需加载指定章节（优先使用）")
  console.log("  - get_skill_summary(name)      获取规范摘要（大纲+预览）")
  console.log("  - get_skill_content(name)      获取规范完整内容（谨慎使用）")
  console.log("  - search_skill(query)          搜索规范（返回章节级定位）")
  console.log("")
