...
**本文档有两种使用场景**：
1. **主动触发**：意图路由识别到"变更已有功能"时，作为 CHANGE REQUEST 流程的规则参考（参见 `core-workflow.md` 的 CR1-CR5）
2. **被动参考**：工作流执行过程中用户临时请求变更时，按本文档的类型和指南处理

---

## 变更入口：快速评估检查表

当从意图路由进入本文档时，按以下顺序执行快速评估：

1. **确认变更目标**：用户想修改哪个功能？（功能名称 + 具体改动）
2. **定位产出物**：该功能在哪些文档和代码中有体现？
3. **判断变更类型**：对照下方"工作流中途变更类型"，确定属于哪种类型
4. **执行对应处理方式**：按类型的处理方式执行

**快速分类决策**：

| 用户描述 | 对应类型 | 处理要点 |
|---------|---------|----------|
| "修改 XX 的某个字段/规则" | 类型 3（修改当前/已完成产物） | 低影响，直接修改 |
| "XX 功能要增加一个新场景" | 类型 8（添加工作单元）或新增追加 | 中影响，追加产物 |
| "XX 功能的整体逻辑要改" | 类型 4（重新开始之前步骤） | 高影响，从需求开始重做 |
| "XX 和 YY 之间的接口要改" | 类型 7（架构变更） | 高影响，级联更新 |
...
2. **更新所有跟踪**：保持 `state.md`、计划文件和 `audit.md` 同步
3. **沟通进度**：让用户了解正在发生的事情
