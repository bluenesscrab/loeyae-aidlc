...
**额外规则**：
1. **不信任会话摘要中的任务列表** — 会话摘要可能包含压缩前的 Pending Tasks，但这些任务可能不是 AI-DLC 流程中的正确下一步
2. **以 state.md 为唯一事实来源** — 当前阶段、当前步骤、下一步操作全部从 state.md 读取
3. **重新加载当前步骤的 steering 文件** — 不依赖压缩前的上下文，确保执行规则完整
4. **检查"下一步交接"表格** — 如果 state.md 中有交接提示词，优先使用该提示词恢复
...
读取 state.md
    ↓
├── 有"下一步交接"提示词？
│     是 → 按提示词恢复（跳过欢迎提示，直接继续）
│     否 → 按 state.md 的"当前步骤"恢复
    ↓
激活对应 skill + 宣布恢复状态
    ↓
从下一步骤继续（不展示完整欢迎提示，直接工作）
```
...
**必须加载**：
1. 该页面的 UI Mock（对应端 HTML 文件中的 mock-box）— 这是该页面的权威完整描述
2. 需求文档中描述该页面功能的完整段落（通过页面名称检索）
3. 所有提及该页面的用户故事（可能分布在多个单元的故事文件中）
4. 该页面的实际代码文件

**加载方法**：
- 在 UI Mock HTML 中搜索页面名称，定位对应 mock-box
  - 标准模式：直接搜索 `ui-mock/{端名}.html`
  - 大型模式：先查看 `ui-mock/{端名}/index.html` 确定模块，再搜索对应模块文件
- 在需求文档中搜索页面名称，定位相关段落
...
- ❌ 忽略后续变更请求中迁入该页面的功能
- ❌ 以旧版 UI Mock 或旧版需求为准进行修改（必须以当前文件内容为准）
