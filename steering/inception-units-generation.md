...
- 允许修改的代码与配置范围；
- 契约、配置、数据和外部系统依赖；
- 涉及的角色正式文档目录、资料修订/片段引用与章节血缘责任；
- 对应 UC-D、完成证据及前置/后置验证检查点。

跨服务资料闭环必须按 Provider/Consumer 所有权拆分：Provider 单元负责资料、索引和契约实现，Consumer 单元负责角色意图、上下文消费、正式文档及引用/血缘调用。AI-DLC 不得直接依赖 Provider Worker 或数据库，Kiro、Claude Code、OpenCode 适配单元不得复制核心业务规则。
