...
**进入 Construction 前，必须检查 Inception 步骤 7（应用设计）和步骤 8（单元生成）是否需要执行且已完成。**

### 检查规则

| Inception 步骤 | 必须执行的条件（满足任一） | 缺失时处理 |
|---------------|--------------------------|-----------|
| 步骤 7 应用设计 | ① 有新接口定义 ② 跨模块/跨服务调用 ③ 多端改动（前+后/多前端） ④ 需定义服务层方法和业务规则 | **阻断**：先补完应用设计再进 Construction |
| 步骤 8 单元生成 | ① 需拆分为多个工作单元 ② 工作量预估 > 单次可完成范围 | **阻断**：先执行单元生成再进 Construction |

### 执行流程

```
用户说"开始 Construction" / "开发接手"
    ↓
检查 state.md：步骤 7/8 是否标记为已完成或已跳过？
    ↓
├── 已完成/已跳过（有记录） → 通过，进入 Construction
├── 条件不满足（确实不需要） → 通过，进入 Construction
└── 条件满足但未执行 → 阻断，提示 Boss 先补完
```

### 阻断提示模板

```markdown
Boss，检测到 Inception 阶段有未完成的必要步骤：

❌ **应用设计**未完成（检测到：[触发条件]）
❌ **单元生成**未完成（检测到：[触发条件]）

这些步骤对代码质量至关重要：
- 应用设计定义了接口契约和服务边界，缺失会导致返工
- 单元生成确保任务拆分合理，缺失会导致上下文溢出

**建议**：先完成以上步骤，再进入 Construction。

A) 补完 [步骤名] — 从中断处继续 Inception
B) 确认跳过 — 我确认当前场景不需要（请说明原因）

[回答]:
```

**注意**：此门禁不受复杂度分级影响。即使是精简流程，满足条件时应用设计和单元生成仍然不可跳过。
...
- **TDD 执行不受复杂度分级影响，精简流程/完整流程/快速通道均必须执行。没有失败测试就没有生产代码。**
