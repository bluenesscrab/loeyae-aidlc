...
**执行**：
1. **必须**：在 audit.md 中记录此阶段的任何用户输入
2. 加载 `inception-ui-mock.md` 的所有步骤
3. 执行 UI Mock：
   - 确定涉及的端
   - 读取现有前端代码（存量项目），判断每个页面是新增还是局部改造
   - 输出页面清单，提交用户确认
   - 逐端制作 HTML Mock（页面内容 + 业务说明）
   - 精简优化（合并重复页面，用条件表格标注差异）
   - 用户逐页审核，修正问题
   - 执行 6 维度交叉审核（需求来源、系统复用、状态完整性、多端一致性、页面vs逻辑分离、操作路径闭环）
4. **必须**：更新 `state.md` 的"下一步交接"字段
5. **等待明确批准** - 用户确认前不得继续
6. **必须**：在 audit.md 中记录用户的完整原始输入
