...
**用途**：可视化参考（Mermaid 流程图）。**不定义规则** — 所有规则定义见 `core-workflow.md`。

**加载时机**：仅在需要向用户展示工作流全貌可视化时加载，不作为常规 steering 文件预加载。

**与其他文件的关系**：
- `core-workflow.md`：规则的权威来源（路由表、审批模式、核心原则）
- **本文件**：规则的可视化展现（Mermaid 图 + 描述性摘要）
- `README.md`：面向人类的仓库文档
