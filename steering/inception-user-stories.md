...
完整性门禁：`personas.md` 每个角色至少有一条矩阵记录；每个 FR 至少关联一条矩阵记录，或进入权限不适用记录；多角色系统必须覆盖只读角色和无权角色。权限来源定位键按 `product-scenario-module-mapping.md` §8 生成，由 I8 审查格式、范围唯一性和 FR 关联。
...
- 角色权限矩阵固定表头、非 UI 规则、FR/角色覆盖和来源定位键均通过完整性检查
- 生成的故事已获用户明确审批
- 故事已验证并准备好进入下一阶段
