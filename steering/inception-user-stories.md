...
需求涉及 SSOT 沟通/过程资料时，故事规划必须遵守：

1. 主旅程按“资料采集 → 解析 → 修订/讨论 → 项目内检索 → 上下文/引用 → 角色化正式文档 → 血缘”组织；逆向说明上传作为适用的派生资料支线。
2. 产品经理、架构师、项目经理、开发人员、测试人员的故事只引用 `common-directory-structure.md` 冻结的核心正式文档目录，不得声称存在独立角色模板。
3. Provider 与 Consumer 分属不同产品时，各自在自身仓维护故事正文，只通过已批准契约 ID、Gherkin 结果和集成验证关联，不镜像对端故事或复制字段级 Schema。
4. 故事计划只能在 I6 通过后形成，并须取得独立明确审批后才能生成 `personas.md` 和 `stories.md`。
5. 平台适配只描述同一业务语义的交互差异；Kiro、Claude Code、OpenCode 不得成为业务项目属性、故事分组或完成门禁。
6. 资料解析、检索、Schema、鉴权、血缘或平台运行尚无执行证据时，故事与计划必须标记未验证，不得以文档审阅代替运行结论。
