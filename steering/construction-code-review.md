...
| 模式 | 适用条件 | 执行方式 |
|------|----------|----------|
| 集成双轴审查 | `common-complexity-assessment.md` 判定为简单快速通道，且为单单元、无快速通道排除项 | 同一审查上下文依次完成 Spec 与 Standards 检查，分别记录结论 |
| 独立双轴审查 | 中等/复杂流程，或多单元、跨组件/服务、契约/共享配置/数据所有权/安全边界风险 | Spec 与 Standards 使用独立视角或独立子 Agent，分别报告 |

复杂度升级后立即切换为独立双轴审查；不得以节省 token 为由降低审查模式。两种模式都必须修复关键/重要问题并复审，但复审只加载修复范围、原问题及必要回归上下文。
