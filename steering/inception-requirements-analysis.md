...
## 步骤 4.1：加载项目资料上下文（配置 SSOT 时）

在不替代步骤 1 的代码/架构事实和步骤 6 的数据源溯源前提下：

1. 根据当前业务项目、执行角色和目标正式文档形成资料意图。
2. 仅检索当前项目资料，默认最新修订；支持显式包含、排除和指定旧版。
3. 范围过大、低置信、资料冲突或索引失败时请求确认，并记录元数据/全文/向量等实际降级方式。
4. 区分已确认资料、未确认讨论、显式结论和模型推断，不得将后两者伪装为权威事实。
5. 在需求产物中保留实际使用的资料修订/片段引用，并建立资料版本/片段到正式文档章节的血缘入口。
6. 未配置 SSOT 时保持 Legacy 本地流程且远程调用为零；客户端平台不得写入业务项目属性或需求事实。
