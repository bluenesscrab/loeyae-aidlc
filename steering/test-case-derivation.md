...
已批准需求、CR 或应用设计包含资料驱动研发闭环时，按实际范围派生并保持未验证状态，至少评估：

| 风险来源 | 至少覆盖 |
|----------|----------|
| 六类文件解析 | Word、Excel、PPT、PDF、Markdown、SQL 的成功、定位、失败与重试 |
| 修订与讨论 | 不可变修订、当前指针、指定旧版、未确认讨论与显式结论 |
| 项目内检索 | 元数据/全文/向量/单项目 GraphRAG、低置信、索引失败可见与降级 |
| 引用与血缘 | 修订/片段固定引用、正式文档章节定位、引用不漂移和双向追踪 |
| 逆向说明 | 仓库路径、Git commit、内容哈希及不覆盖代码事实 |
| 安全与适配 | 项目隔离/越权负向路径，以及产品级三平台语义一致性 |

文档审阅只能证明来源和计划完整，不得标记上述运行行为通过；实际证据仍须包含稳定运行标识、不可变代码版本、范围、结果和报告位置。
