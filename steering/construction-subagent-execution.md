...
**平台自适应**：检测当前平台是否支持子 Agent 派发。审查模式先由 `construction-code-review.md` 按复杂度与风险确定，再选择子 Agent 或单 Agent 的执行方式；平台能力不得改变质量门禁。
...
集成模式只派发一个审查者，在同一上下文内依次完成 Spec 与 Standards 并分别报告；独立模式使用两个审查视角或两个审查者。实现者自审不能替代正式双轴检查。
...
以下流程描述独立双轴审查。集成模式将规格审查与质量审查合并为一个审查子 Agent 调用，但仍依次完成两组检查、分别报告结论；修复后只传递原问题、修复差异及必要回归上下文。
...
│     标记任务完成                                             │
│                                                             │
│  3. 所有任务完成后：按 C7 条件决定是否派发最终审查子 Agent       │
│  4. 进入构建和测试阶段                                       │
└─────────────────────────────────────────────────────────────┘
```
...
按 `construction-code-review.md` 阶段 1 的四维度执行：完整性、准确性、无多余、接口契约。
...
按 `construction-code-review.md` 阶段 2 的三级分类执行：关键（必须修复）、重要（应该修复）、建议（可选修复）。
