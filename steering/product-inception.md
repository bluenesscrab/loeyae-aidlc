...
更新 state.md 的产品级进度、活跃层级、基线引用和下一步模块选择。

## 资料驱动的角色与正式文档边界

产品级 Inception 涉及沟通/过程资料与正式研发文档时，必须同时执行以下约束：

1. 以产品经理、架构师、项目经理、开发人员、测试人员五类角色及目标文档建立产品级目录索引，具体落盘位置使用 `common-directory-structure.md` 的既有阶段目录，不创建角色专属平行目录。
2. 沟通/过程资料及逆向工程派生说明可由 SSOT 权威管理；正式研发文档正文和批准版本仍由项目工作区/Git 或既有文档库权威管理；源代码和代码事实始终以 Git 为准。
3. 产品级契约索引只登记项目解析、资料/修订/片段读取、项目内检索、上下文包、逆向说明上传、引用/章节血缘写回、索引状态和失败降级，不复制 Provider 字段级 Schema。
4. 任务、人力、实际进度、CI/测试执行、制品和部署事实继续由外部专业平台权威管理，不得根据资料推断为已执行。
5. 客户端平台只属于产品适配范围，不得成为业务项目属性、正式文档目录或审批门禁。
